...
export const FlightListCard = (flight: Flight) => {
	return (
		<div
			className="flex lg:flex-col gap-6 w-full bg-white p-6 text-blackishGreen rounded-[12px]"
...
const FavoriteButton = ({ flight }: { flight: Flight }) => {
	const { isFavorite, toggleFavorite, favorites } = useFlightFavorites();
	...
	return (
		...
		>
			<HeartIcon
				fill={isFavorite(flight.id) ? "black" : "none"}
				stroke={isFavorite(flight.id) ? "" : "#4C4850"}
			/>
		</button>
	);
};
